...
import dev.anye.sdtd.helper.config.*;
...
public abstract class ServerConfig {
    ...
    public final CommandPoint commandPoint;

    public final HashMap<String,String> js = new HashMap<>();
    ...
    public ServerConfig(ServerData serverData){
        ...
        $7DTD._Log.info("开始加载命令积分文件");
        commandPoint = new CommandPoint(getConfigPre() + "CommandPoint.json");

        $7DTD._Log.info("开始加载Js文件");
        JsEnableConfig j = new JsEnableConfig(getConfigPre() + "Js.json");
        j.getDatas().forEach((s, aBoolean) -> {
            if (aBoolean && !JsCommands.I.getCommand(s).isEmpty()){
                js.put(JsCommands.I.getCommand(s),s);
            }
        });
        $7DTD._Log.info("已加载["+js.size()+"]js脚本");
    }
